...
def _best_carpet_move(bs):
    ...
    for d in DIRS:
        ...
        base = CARPET_POINTS_TABLE.get(r, 0)
        if base <= 0:
            continue

        # NEW: compare against stepping forward instead of carpet
        nx, ny = _step(x, y, d)
        if bs.is_valid_cell((nx, ny)):
            # if moving forward gives similar or better belief gain,
            # skip carpet
            forward_gain = 6.0 * float(belief_global_proxy(bs, nx, ny))

            if forward_gain > base:
                continue
    ...
    def play(self, bs, sensor, time_left):
        ...
        if current_cell == Cell.SPACE:
            ...
            if best_d is not None:
                expected = CARPET_POINTS_TABLE.get(min(best_r, 7), 0)

                if best_r >= 3 and expected >= 4:
                    c = Move.prime(best_d)
